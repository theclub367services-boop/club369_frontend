...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ isLoading }) => {
  ...
  // Spring-smooth progress bar — same technique as Dashboard ProgressBar
  const rawProgress    = useMotionValue(0);
  const springProgress = useSpring(rawProgress, { stiffness: 60, damping: 22, mass: 1 });
  const barScaleX      = useTransform(springProgress, [0, 100], [0, 1]);

  // Simulate realistic stepped loading (network + parse + render phases)
  useEffect(() => {
    if (!isLoading) {
      rawProgress.set(100);
      setProgress(100);
    ...
    // Stepped progress: fast to 40%, slower to 80%, stalls until real load completes
    ...
    <AnimatePresence>
      {phase === 'loading' && (
        <motion.div
          ...
          exit={{
            opacity: 0,
            // scale-out on dismiss — acceptable here since it's a one-time full-screen exit
            scale: 1.04,
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none" aria-hidden>
            ...
            />
            <div
              className="absolute bottom-1/4 left-1/4 w-[250px] h-[250px]
                         bg-blue-900/6 rounded-full blur-[90px]"
              style={{ transform: 'translateZ(0)', backfaceVisibility: 'hidden' }}
            />
          </div>

          {/* ── Rotating ring — single GPU element ── */}
          ...
          <div className="relative z-10 flex flex-col items-center gap-8">
            ...
            >
              <h1 className="text-2xl font-bold tracking-[0.4em] text-white uppercase
              ...
              <p className="text-[10px] font-bold tracking-[0.35em] text-gray-500 uppercase mt-1
                            [-webkit-font-smoothing:antialiased]">
              ...
              <div className="w-full h-[2px] bg-white/5 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className="h-full w-full bg-gradient-to-r from-primary via-purple-400 to-primary
                             ...
                             shadow-[0_0_8px_rgba(175,37,244,0.8)]"
                />
              </div>
          ...
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-[18vw] font-bold
